...
We are currently working on the extension of the package to linear and logistic regression.Theoretical background is provided in the Appendix at the end of this vignette.
...
A third function `plotINT` is also implemented to provide a graphical display of the results and only require the `rcsHR` or `loglinHR` results as object. 
...
$OR_{10}=\frac{\exp(\beta_0+\beta_1+sp(z)+sp_2(z))}{\beta_0+\exp(sp(z))}$  ,
