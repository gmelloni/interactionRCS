---
title: "hrcubspline_more_than_3_knots"
author: "Hong Xiong"
date: "2023-06-08"
output: pdf_document
---
...
rcsHR_knot_more_than_3 <- function(var2values , model , data=NULL , var1 , var2
...
myformula3 <- Surv(time, censor) ~ treat*rcs(age, 3) + site + nonwhite + ivdrug
# Find the 3 knots to test if hardcoding works
my3k <- attributes(rms::rcs(umaru$age , 3))$parms
myformula3hard <- Surv(time, censor) ~ treat*rcs(age, c(24, 32, 40)) + site + nonwhite + ivdrug
# Try both cph and coxph
mymodel3 <- list(
  model3_cph = cph(myformula3 , data = umaru , x = TRUE , y=TRUE)
  ,model3_coxph = coxph(myformula3 , data = umaru , x = TRUE , y=TRUE)
  ,model3_cph_hard = cph(myformula3hard , data = umaru , x = TRUE , y=TRUE)
  ,model3_coxph_hard = coxph(myformula3hard , data = umaru , x = TRUE , y=TRUE)
)
# Set boundaries
maxmin <- min(umaru$age):max(umaru$age)
# Original
HR_rcs_delta <- lapply(names(mymodel3[c(1,2)]), function(modmod) {
  rcsHR( var2values = maxmin , model = mymodel3[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "delta")
}) %>% setNames(c("Original CPH" , "Original COXPH"))
# New
HR_rcs_delta2 <- lapply(names(mymodel3) , function(modmod) {
  rcsHR_knot_more_than_3( var2values = maxmin
                     , model = mymodel3[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "delta")
}) %>% setNames(names(mymodel3))

pdf("knots3_delta.pdf" , width = 8 , height = 12)
par(mfrow=c(4,2))
for(i in names(HR_rcs_delta)){
  plotINT2(HR_rcs_delta[[i]]  , main = i , ylim = c(0,3))
}
for(i in names(HR_rcs_delta2)){
  plotINT2(HR_rcs_delta2[[i]]  , main = i , ylim = c(0,3))
}
dev.off()
...
# Original
HR_rcs_delta <- lapply(names(mymodel3[c(1,2)]), function(modmod) {
  rcsHR( var2values = maxmin , model = mymodel3[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "bootstrap")
}) %>% setNames(c("Original CPH" , "Original COXPH"))
# New
HR_rcs_delta2 <- lapply(c(names(mymodel3[c(1,2)])) , function(modmod) {
  rcsHR_knot_more_than_3( var2values = maxmin
                     , model = mymodel3[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "bootstrap" , fixedknot = FALSE)
}) %>% setNames(c("New CPH" , "New COXPH"))
HR_rcs_delta3 <- lapply(c(names(mymodel3[c(1,2)])) , function(modmod) {
  rcsHR_knot_more_than_3( var2values = maxmin
                     , model = mymodel3[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "bootstrap" , fixedknot = TRUE)
}) %>% setNames(c("New CPH knot fixed" , "New COXPH knot fixed"))

pdf("knots3_bootstrap.pdf" , width = 8 , height = 12)
par(mfrow=c(4,2))
for(i in names(HR_rcs_delta)){
  plotINT2(HR_rcs_delta[[i]]  , main = i , ylim = c(0,3))
}
...
for(i in names(HR_rcs_delta3)){
  plotINT2(HR_rcs_delta3[[i]]  , main = i , ylim = c(0,3))
}
dev.off()

# 4 knots
## 4 Knots DELTA result
myformula4 <- Surv(time, censor) ~ treat*rcs(age, 4) + site + nonwhite + ivdrug
# Find the 4 knots to test if hardcoding works
my4k <- attributes(rms::rcs(umaru$age , 4))$parms
myformula4hard <- Surv(time, censor) ~ treat*rcs(age, c(23, 29, 35 , 43)) + site + nonwhite + ivdrug
# Try both cph and coxph
mymodel4 <- list(
  model4_cph = cph(myformula4 , data = umaru , x = TRUE , y=TRUE)
  ,model4_coxph = coxph(myformula4 , data = umaru , x = TRUE , y=TRUE)
  ,model4_cph_hard = cph(myformula4hard , data = umaru , x = TRUE , y=TRUE)
  ,model4_coxph_hard = coxph(myformula4hard , data = umaru , x = TRUE , y=TRUE)
)
# Set boundaries
maxmin <- min(umaru$age):max(umaru$age)
# New
HR_rcs_delta2 <- lapply(names(mymodel4) , function(modmod) {
  rcsHR_knot_more_than_3( var2values = maxmin
                     , model = mymodel4[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "delta")
}) %>% setNames(names(mymodel4))

pdf("knots4_delta.pdf" , width = 8 , height = 12)
par(mfrow=c(2,2))
for(i in names(HR_rcs_delta2)){
  plotINT2(HR_rcs_delta2[[i]]  , main = i , ylim = c(0,3))
}
...
# Bootstrap
HR_rcs_delta2 <- lapply(names(mymodel4)[1:2] , function(modmod) {
  rcsHR_knot_more_than_3( var2values = maxmin
                     , model = mymodel4[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "bootstrap")
}) %>% setNames(c("New CPH" , "New COXPH"))
HR_rcs_delta2f <- lapply(names(mymodel4)[1:2] , function(modmod) {
  rcsHR_knot_more_than_3( var2values = maxmin
                     , model = mymodel4[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "bootstrap" , fixedknot = TRUE)
}) %>% setNames(c("New CPH knot fixed" , "New COXPH knot fixed"))

pdf("knots4_bootstrap.pdf" , width = 8 , height = 12)
par(mfrow=c(2,2))
for(i in names(HR_rcs_delta2)){
  plotINT2(HR_rcs_delta2[[i]]  , main = i , ylim = c(0,3))
}
for(i in names(HR_rcs_delta2f)){
  plotINT2(HR_rcs_delta2f[[i]]  , main = i , ylim = c(0,3))
}
dev.off()

# 5 knots
## 5 Knots DELTA result
myformula5 <- Surv(time, censor) ~ treat*rcs(age, 5) + site + nonwhite + ivdrug
# Find the 5 knots to test if hardcoding works
my5k <- attributes(rms::rcs(umaru$age , 5))$parms
myformula5hard <- Surv(time, censor) ~ treat*rcs(age, c(23, 28, 32 , 36 , 43)) + site + nonwhite + ivdrug
# Try both cph and coxph
mymodel5 <- list(
  model5_cph = cph(myformula5 , data = umaru , x = TRUE , y=TRUE)
  ,model5_coxph = coxph(myformula5 , data = umaru , x = TRUE , y=TRUE)
  ,model5_cph_hard = cph(myformula5hard , data = umaru , x = TRUE , y=TRUE)
  ,model5_coxph_hard = coxph(myformula5hard , data = umaru , x = TRUE , y=TRUE)
)
# Set boundaries
maxmin <- min(umaru$age):max(umaru$age)

HR_rcs_delta2 <- lapply(names(mymodel5) , function(modmod) {
  rcsHR_knot_more_than_3( var2values = maxmin
                     , model = mymodel5[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "delta")
}) %>% setNames(names(mymodel5))

pdf("knots5_delta.pdf" , width = 8 , height = 12)
par(mfrow=c(2,2))
for(i in names(HR_rcs_delta2)){
  plotINT2(HR_rcs_delta2[[i]]  , main = i , ylim = c(0,3))
}
...
# Bootstrap
HR_rcs_delta2 <- lapply(names(mymodel5)[1:2] , function(modmod) {
  rcsHR_knot_more_than_3( var2values = maxmin
                     , model = mymodel5[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "bootstrap")
}) %>% setNames(c("New CPH" , "New COXPH"))
HR_rcs_delta2f <- lapply(names(mymodel5)[1:2] , function(modmod) {
  rcsHR_knot_more_than_3( var2values = maxmin
                     , model = mymodel5[[modmod]] , data = umaru , var1 ="treat", var2="age" ,ci.method = "bootstrap" , fixedknot = TRUE)
}) %>% setNames(c("New CPH knot fixed" , "New COXPH knot fixed"))

pdf("knots5_bootstrap.pdf" , width = 8 , height = 12)
par(mfrow=c(2,2))
for(i in names(HR_rcs_delta2)){
  plotINT2(HR_rcs_delta2[[i]]  , main = i , ylim = c(0,3))
}
