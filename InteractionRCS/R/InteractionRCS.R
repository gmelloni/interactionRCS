# RCSHR
## Bootstrapping Helper Function (Old)
.bootrcsHR <- function(data, idx , x , model , var1 , var2){
  ...
  if("cph" %in% class(mymodel) | "lrm" %in% class(mymodel)){
    separator <- " * "
...
.bootrcsOR_knot_more_than_3 <- function(data, idx , x , model , var1 , var2){
  ...
  ### Normalize variables to avoid high collinearity
  adjustment <- mean(data[[var2]])
  x <- x - mean(data[[var2]])
  data[var1] <- data[var1] - mean(data[[var1]])
  data[var2] <- data[var2] - mean(data[[var2]])
  ...
  ### If hard-coded, need to normalize hard-coded knots

  if(matches_vector_rcs){

    rcs_match <- str_match(form_str, "rcs\\(([^)]*)\\)")

    param_str <- rcs_match[1, 2]

    var <- str_extract(param_str, "^[^,]*")
    coeffs_str <- gsub(".*c\\(", "", param_str)
    coeffs_str <- gsub("\\)", "", coeffs_str)

    coeffs <- as.integer(strsplit(coeffs_str, ", ")[[1]])

    coeffs <- coeffs - adjustment

    new_rcs <- paste0("rcs(", var, ", c(", paste(coeffs, collapse = ", "), ")")

    new_form_str <- str_replace(form_str, "rcs\\(([^)]*)\\)", new_rcs)

    myformula <- as.formula(new_form_str)
  }
