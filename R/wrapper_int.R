...
#'
#' This function is a dispatcher that generate OR, HR, RR or linear estimates values for a simple or restricted cubic spline
#' interaction model from a logistic, Cox, Poisson or linear regression
#'
#' @param var2values numeric vector of var2 points to estimate
#' @param model model of class cph, coxph, lrm, glm or Glm (for glm/Glm, family gaussian, binomial, poisson or quasipoisson). If data is NULL, the function expects to find the data in model$x.
...
#' @return if ci = FALSE, a dataframe with initial values and OR/HR/RR/linear estimates
#' , if ci = TRUE a dataframe with 5 columns, initial values, OR/HR/RR/linear estimates, lower CI, upper CI and SE
...
intEST <- function(var2values , model , data , var1 , var2
  ...
  if(any(c("cph" , "coxph") %in% modelClass)){
    ...
    if(family %in% c("gaussian","quasi")){
      ...
      # Poisson models return rate ratios (RR) via the OR machinery
      if(isRCS){
        argg[[1]] <- as.name("rcsOR")
      } else {
        argg[[1]] <- as.name("loglinOR")
      ...
      stop("glm or Glm object not of family gaussian, binomial nor poisson")
    }
